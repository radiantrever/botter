        ...
        if (expiredSubs.length > 0) {
            ...
            for (const sub of expiredSubs) {
                try {
                    ...
                    if (sub.inviteLink) {
                        await bot.api.revokeChatInviteLink(Number(channelId), sub.inviteLink).catch(() => {});
                    }

                    await subRepo.expireSubscription(sub.id);
                    const message = t(lang, "sub_expired", { channel: sub.plan.channel.title });
                    await bot.api.sendMessage(Number(userId), message, { parse_mode: "Markdown" }).catch(() => { });
                    ...
                    ).catch((err: any) => console.error("Failed to log kick to admin channel:", err));
...
async function sendReminders(days: number, translationKey: "reminder_1d" | "reminder_3d") {
    try {
        const subs = await subRepo.findSubscriptionsForReminder(days);
        for (const sub of subs) {
            const lang = (sub.user as any).language as Language || 'en';
            const message = t(lang, translationKey, { channel: sub.plan.channel.title });
            
            await bot.api.sendMessage(Number(sub.user.telegramId), message, { parse_mode: "Markdown" })
                .then(async () => {
