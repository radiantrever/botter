...
export async function handleStart(ctx: MyContextWithI18n, payload: string | undefined) {
    if (!ctx.from) return;
...
composer.callbackQuery("how_to_sub", async (ctx) => {
    ...
    await ctx.editMessageText(context.t("how_to_sub_text"), {
        parse_mode: "Markdown",
        reply_markup: new InlineKeyboard().text(context.t("back_dashboard_btn"), "main_menu")
    });
});
