...
export class LoggerService {
  ...
  static async logPayoutRequest(details: {
    ...
    if (!channelId) {
      console.warn(
        'LOG_CHANNEL_ID not set in .env. Skipping Telegram notification.'
      );
      return;
    }
    ...
    try {
      ...
      console.error(
        'Failed to send payout notification to Telegram channel:',
        err
      );
    }
  }
  ...
  static async logEvent(message: string) {
    ...
    try {
      ...
      console.error(
        'Failed to send event notification to Telegram channel:',
        err
      );
    }
  }
