...
const creator_service_1 = require("../../core/creator.service");
...
composer.callbackQuery(/approve_partner_(\d+)/, async (ctx) => {
    ...
    try {
        ...
        await ctx.editMessageText(context.t('partner_approved'), {
            reply_markup: new grammy_1.InlineKeyboard().text(context.t('back_dashboard_btn'), 'manage_partners'),
        });
        // Notify Partner
...
composer.callbackQuery('analytics', async (ctx) => {
    ...
    try {
        ...
        const keyboard = new grammy_1.InlineKeyboard().text(context.t('back_dashboard_btn'), 'dashboard');
        await ctx.editMessageText(text, {
            reply_markup: keyboard,
            parse_mode: 'Markdown',
        });
...
composer.callbackQuery('add_channel_info', async (ctx) => {
    ...
    await ctx.editMessageText(context.t('no_channels'), {
        parse_mode: 'Markdown',
        reply_markup: new grammy_1.InlineKeyboard().text(context.t('back_dashboard_btn'), 'dashboard'),
    });
});
...
composer.callbackQuery('payout_history', async (ctx) => {
    ...
    try {
        ...
        const keyboard = new grammy_1.InlineKeyboard().text(context.t('back_dashboard_btn'), 'wallet');
        await ctx.editMessageText(text, {
            reply_markup: keyboard,
            parse_mode: 'Markdown',
        });
...
composer.callbackQuery(/get_link_(\d+)/, async (ctx) => {
    ...
    await ctx.reply(context.t('your_link_title', { link }), {
        parse_mode: 'Markdown',
        reply_markup: new grammy_1.InlineKeyboard().text(context.t('back_dashboard_btn'), `manage_channel_${channelId}`),
    });
});
composer.command('register', async (ctx) => {
    ...
    if (!ctx.from)
        return;
    try {
        await creatorService.registerCreator(BigInt(ctx.from.id), ctx.from.username, ctx.from.first_name, ctx.from.last_name);
        await ctx.reply(context.t('reg_success'), {
            parse_mode: 'Markdown',
            reply_markup: new grammy_1.InlineKeyboard().text(context.t('go_dashboard_btn'), 'dashboard'),
        });
    }
    catch (e) {
        console.error(e);
...
composer.on('message:forward_origin:channel', async (ctx) => {
    ...
    try {
        ...
        // Register
        const creator = await creatorService.registerCreator(BigInt(ctx.from.id), ctx.from.username, ctx.from.first_name, ctx.from.last_name);
        if (!creator)
            throw new Error('Could not register creator.');
        ...
        console.error(e);
        await ctx.reply(`${context.t('error_loading')}: ${e.message}`);
    }
});
...
composer.on('message:text', async (ctx, next) => {
    ...
    if (step === 'withdrawing_amount') {
        ...
        const balance = await creatorService.getBalance(BigInt(ctx.from.id));
        if (amount > balance)
            return ctx.reply(context.t('insufficient_balance'));
...
exports.creatorHandler = composer;
